...
class UserService {
  ...
  ///
  /// Validates the stored token against the API (`GET /auth/me`).
  /// If the token is missing, expired, or rejected, SharedPreferences is
  /// cleared and `false` is returned so the splash routes to sign-in.
  Future<bool> isLoggedIn() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Token rejected (expired / invalid) — wipe local data
        await prefs.clear();
        return false;
      }
    } catch (_) {
      // Network error or timeout — treat as not logged in
      return false;
    }
  }
